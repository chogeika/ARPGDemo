...
public:
	...
	void OnSelectionUpdated(TSharedPtr<FSCSEditorTreeNode> SelectedNode)
	{
		...
		if (BlueprintEditor.IsValid())
		{
			...
			{
				//fix me
					//EditingComponent = FSCSEditorTreeNode::GetEditableComponentTemplate(true);
...
void FAFEffectCueDetails::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	if (DetailsView)
	{
		...
		if (HostTabManager.IsValid() && HostTabManager->HasTabSpawner("EmbeddedEffectCueSequenceID"))
		{
